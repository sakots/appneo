...
(() => {
  ...
  type AppNeoState = {
    ...
    neoBaseUrl: string;
    neoBaseUrlPromise: Promise<string> | null;
  ...
  const configureNeoParams = (sizes: Sizes, boardParams: BoardParams) => {
    ...
    const plural =
      Neo.params && typeof Neo.params === "object" ? Neo.params : {};
    ...
    const params = {
      ...
      url_save: getAbsoluteBoardUrl(
        existingPaintBbs.url_save,
        boardParams.url_save,
      ),
      url_exit: getAbsoluteBoardUrl(
        existingPaintBbs.url_exit,
  ...
  class AppNeoFitManager {
    ...
    getExpandedWidth() {
      ...
      const paletteWidth = this.palette
        ? this.palette.getBoundingClientRect().width
        : 0;
  ...
  class AppNeoPaletteManager {
    ...
    async PaletteRenew() {
      if (!document.paintbbs || !this.select || this.select.selectedIndex < 0)
        return;
      this.Palettes[this.select.selectedIndex] = String(
        await document.paintbbs.getColors(),
      );
      this.PaletteListSetColor();
    }
  ...
  const resolveLatestNeoBaseUrl = (): Promise<string> => {
